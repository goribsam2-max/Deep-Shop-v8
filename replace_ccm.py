...
pattern = r'\{/\* --- Create Community Channel Modal --- \*/\}.*?(?=\{/\* --- Review Details Modal --- \*/\})'
...
if match:
    content = content[:match.start()] + new_create_channel + content[match.end():]
    with open('pages/Messages.tsx', 'w', encoding='utf8') as f:
        f.write(content)
    print("Replaced create channel modal")
else:
