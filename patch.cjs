...
const target = `                             <div key={msg.id} className={\`flex gap-2 \${isMe ? 'justify-end' : 'justify-start'} relative pb-3\`} onClick={() => setActiveMessageMenuId(null)}>`;
const replace = `                             <div key={msg.id} id={\`msg-\${msg.id}\`} className={\`flex gap-2 \${isMe ? 'justify-end' : 'justify-start'} relative pb-3 group\`} onClick={() => setActiveMessageMenuId(null)}>`;
code = code.replace(target, replace);

const target2 = `                                     <div 
                                         className={\`absolute -bottom-2 \${isMe ? 'right-4' : 'left-4'} flex items-center gap-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm rounded-full px-1.5 py-0.5 z-10 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition cursor-pointer \${reactionEntries.length > 0 ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}\`} 
...
const target3 = `                                     <div className="max-w-[75%] items-start flex flex-col relative">`;
const replace3 = `                                     <div className="max-w-[75%] items-start flex flex-col relative group/bubble">`;
