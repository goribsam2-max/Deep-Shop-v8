...
class AudioService {
  ...
  private initCtx() {
    ...
    if (this.ctx) {
      ...
      // Auto-resume context on document interaction to bypass strict browser autoplay policies
      const resumeHandler = () => {
        if (this.ctx && this.ctx.state === 'suspended') {
          this.ctx.resume().catch(e => console.error("Audio Context auto-resume failed:", e));
        }
      };
  ...
  play(type: string = 'ringing', loop?: boolean) {
    if (this.muted) return;
    if (type === 'calling') {
      this.playCalling();
  ...
  playNotification() {
    ...
    try {
      ...
      // High-quality double-tap synth chime
  ...
  playCalling() {
    ...
    const playBeepPair = () => {
      if (this.muted || !this.ctx || this.soundType !== 'calling') return;
  ...
  playRingtone() {
    ...
    const playMelody = () => {
      if (this.muted || !this.ctx || this.soundType !== 'ringing') return;
