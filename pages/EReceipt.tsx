  ...
  return (
    <div className="p-6 animate-fade-in min-h-screen bg-zinc-50 dark:bg-zinc-800 max-w-md mx-auto print:p-0">
      ...
      >
              ...
              {order.advanceAmount > 0 && (
                <>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-400 mt-2">
                    ...
                    <span className="text-zinc-900 dark:text-zinc-100">-{formatPrice(order.advanceAmount)}</span>
                  </div>
                  {order.courierPaymentDetails && (
                    <div className="flex justify-between text-[10px] font-bold text-zinc-400 mt-1">
                      <span>Courier Due Paid</span>
                      <span className="text-zinc-900 dark:text-zinc-100">-{formatPrice(order.courierPaymentDetails.amount)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm font-bold pt-2 border-t border-dashed border-zinc-200 dark:border-zinc-700 tracking-tight text-rose-600 mt-2">
                    <span>Due Amount (COD)</span>
                    <span>{formatPrice(Math.max(0, order.total - order.advanceAmount - (order.courierPaymentDetails?.amount || 0)))}</span>
                  </div>
                </>
              )}
            </>
        ...
        <div className="w-full text-center">
          <p className="text-[8px] font-semibold  tracking-normal text-zinc-300">
            Thank you for shopping at DEEP SHOP
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full mt-10 py-5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-full flex items-center justify-center space-x-3 text-[10px] font-semibold tracking-normal print:hidden shadow-lg active:scale-[0.98] transition-all"
      >
        <Icon name="print" />
