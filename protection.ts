export function setupProtection() {
  // Bypass all aggressive blocking/anti-debugging in development and preview environments
  ...
  if (isDev) {
    console.log("Protection bypassed in development/preview environment.");
    return;
  }
  ...
  if (suspiciousBots.some((bot) => ua.includes(bot))) {
    // Subtle console warning for bots instead of breaking the page completely
    console.warn("AI crawler detected.");
    return;
  }
  ...
  // DevTools detection loop removed to prevent lag/blank screen issues on slow devices.
